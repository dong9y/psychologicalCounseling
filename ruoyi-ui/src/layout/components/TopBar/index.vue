...
export default {
  ...
  computed: {
    ...
    topMenus() {
      return this.$store.state.permission.sidebarRouters.filter((f) => !f.hidden && !this.hiddenMenuTitles.includes(f.meta && f.meta.title)).slice(0, this.visibleNumber)
    },
    moreRoutes() {
      const sidebarRouters = this.$store.state.permission.sidebarRouters;
      return sidebarRouters.filter((f) => !f.hidden && !this.hiddenMenuTitles.includes(f.meta && f.meta.title)).slice(this.visibleNumber, sidebarRouters.length - this.visibleNumber)
    },
    hiddenMenuTitles() {
      return ["若依官网", "系统工具"]
    },
